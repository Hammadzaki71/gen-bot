...
async def on_ready():
    ...
    print("LinkGen Ready", "\n")
    await client.change_presence(activity=discord.Game(name="IceGen V2.0"))
                ...
                if str(json.loads(open("config.json", "r").read())["gen_role"]) in str(ctx.author.roles):
                    if os.path.exists(f"accounts/{service.lower()}.txt"):
                        with open(f"accounts/{service.lower()}.txt", "r+") as accounts:
                            ...
                            try:
                                embed = discord.Embed(title=f"{service} Account Generated", description="Ice Generator", color=0x46a9f0)
                                ...
                                embed = discord.Embed(title=f"** {service} account generated successfully**", description=":Arrow: Check Your Dms for Your Account", color=0x46a9f0)
                                embed.set_thumbnail(url="")
                                embed.set_image(url="https://media.discordapp.net/attachments/773934853599002684/904462591861092422/standard_1.gif")
...
async def help(ctx):
    ...
    embed.set_footer(text="Made by Me")
    await ctx.respond(embed=embed)
...
async def stock(ctx):
    ...
    for service in services:
        if os.path.exists(f"accounts/{service.lower()}.txt"):
            stocklist.append(f"** {service} stock**: ``{len(open(f'accounts/{service}.txt', 'r').readlines())}`` ")
    embed = discord.Embed(title="IceGen Stock", description="Ice Stock | Free", color=0x46a9f0)
...
@client.slash_command(name="pgen", guild_ids=[json.loads(open("config.json", "r").read())["guild_id"]])
async def generate(ctx, service_name):
...
async def stock(ctx):
    ...
    stocklist = []
    for service in services:
        if os.path.exists(f"pstock/{service.lower()}.txt"):
            stocklist.append(f"** {service} stock**: ``{len(open(f'pstock/{service}.txt', 'r').readlines())}`` ")
    embed = discord.Embed(title="IceGen Stock", description="Ice Stock | Premium", color=0x46a9f0)
    embed.add_field(name="Stock", value="\n".join(stocklist))
    embed.set_footer(text="Made by Me")
    await ctx.respond(embed=embed)
